...
test("unit: createGroupMeta validates and dedupes", () => {
  const g = createGroupMeta({ name: "Engineering", memberIds: ["alpha", "beta"], description: "x" });
  assert.equal(g.name, "Engineering");
  assert.equal(g.memberIds.length, 2);

  const dup = createGroupMeta({ name: "Dup", memberIds: ["alpha", "alpha", "beta"] });
  assert.deepEqual(dup.memberIds, ["alpha", "beta"]);

  assert.throws(() => createGroupMeta({ name: "" }), /required/);
  assert.throws(() => createGroupMeta({ name: "ok", memberIds: ["BAD CAPS"] }), /Invalid member/);
  assert.throws(() => createGroupMeta({ name: "ok", memberIds: "not-array" }), /must be an array/);
});
...
test("security: logic buildFanOut command substitution does NOT execute", () => {
  ...
  try {
    ...
    const { fanOutCommands } = buildFanOut({ group, senderName: "alpha", content: malicious });
    assert.equal(fanOutCommands.length, 1);
    const argv = runInShell(fanOutCommands[0].cliCommand, dir);
    assert.ok(!existsSync(canary), "no shell expansion or chained command may execute");
...
test("security: logic argv is shell-free and shell string is single-quoted", async () => {
  ...
  const group = createGroupMeta({ name: "Team", memberIds: ["alpha", "beta"] });
  const { fanOutCommands } = buildFanOut({ group, senderName: "alpha", content: "hello $(id)" });
  const cmd = fanOutCommands[0];
  // Argv form must be literal, no quoting.
  assert.deepEqual(cmd.argv, ["-p", "beta", "chat", "--in", "~", "-c", "[Room: Team]", "-Q", "-q", `[Room: Team] 🤖 alpha (@alpha): hello $(id)`]);
  // Shell form must be single-quoted via shellQuote, not JSON.stringify.
  assert.ok(cmd.cliCommand.includes(shellQuote("[Room: Team]")));
  assert.ok(!cmd.cliCommand.includes('"' + "[Room: Team]" + '"') || cmd.cliCommand.includes("'"));
});

test("unit: buildFanOut rejects non-member sender", () => {
  const group = createGroupMeta({ name: "Team", memberIds: ["alpha", "beta"] });
  assert.throws(() => buildFanOut({ group, senderName: "gamma", content: "hi" }), /not a member/);
});
