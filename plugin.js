...
export default {
  ...
  register(ctx) {
    ...
    // Current plugin storage is synchronous JSON state. Hydrate before any
    // contribution renders so a fast Create cannot overwrite stored Teams.
    try {
      const meta = ctx.storage?.get?.('bot-meta', null)
      if (meta && typeof meta === 'object') $botMeta.set(meta)
      $teams.set(normalizeTeams(ctx.storage?.get?.(TEAM_STORE_KEY, { version: TEAM_STORE_VERSION, teams: [] })))
      teamSessions = normalizeTeamSessions(ctx.storage?.get?.(TEAM_SESSION_STORE_KEY, {}))
    } catch {
      $teams.set([])
      teamSessions = {}
    }
